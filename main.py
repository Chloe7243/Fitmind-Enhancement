    ...
    if request.method == "POST":
        with app.app_context():
            try:
                ...
                if user_data is None or not user_data.check_password(
                ...
                flash(
                    "An unexpected error occurred when adding your account to our system"
                )
                flash(f"{e}")
                return render_template("login.html")
    ...
    if request.method == "POST":
        ...
        with app.app_context():
            try:
                ...
                username_presence_check = UserAccounts.query.filter_by(
                    username=username
                ).first()
                email_presence_check = UserAccounts.query.filter_by(
                    username=email
                ).first()
                if (
                    username_presence_check is not None
                    or email_presence_check is not None
                ):
...
def delete_note(id):
    if request.method == "GET":
        with app.app_context():
            stress_log = Logs.query.filter_by(
                log_id=id, made_by=current_user.id
            ).first()
            if stress_log:
...
def exercise():
    ...
    if request.method == "POST":
        if request.is_json:
            ...
            return jsonify([{"type": exercise, "duration": duration, "time": now}]), 200
        ...
        with app.app_context():
            try:
                ...
                note = Notes(
                    made_by=current_user.id,
                    title=note_title,
                    user_description=note_description,
                )
                db.session.add(note)
                db.session.commit()
    ...
    if request.method == "POST":
        with app.app_context():
            try:
                ...
                update_query = (
                    update(UserAccounts)
                    .where(UserAccounts.id == current_user.id)
                    .values(profile_image=string_for_database)
                )
